...
class MigrationTest {
    @Test fun versionFourUpgradeChargesExistingPeopleAndPreservesTrial() = runBlocking {
        ...
        val schema = JSONObject(InstrumentationRegistry.getInstrumentation().context.assets
            .open("currency-v4.json").bufferedReader().use { it.readText() }).getJSONObject("database")
        SQLiteDatabase.openOrCreateDatabase(path, null).use { old ->
            ...
            old.execSQL("INSERT INTO activities VALUES (1, ' PEOPLE ', 'NEUTRAL', 1, 1), (2, 'Renamed project', 'WORK', 1, 0)")
            old.execSQL("INSERT INTO records VALUES (1, 1, 1000, 3001000, '')")
            old.execSQL("INSERT INTO trial VALUES (1, 1000, 0, 1000, 3000000, 3000000, 0, 'Europe/Rome')")
        ...
        val db = Room.databaseBuilder(context, CurrencyDatabase::class.java, name)
            .addMigrations(CurrencyDatabase.MIGRATION_4_5).build()
        try {
            val stored = CurrencyRepository(db).load()
            assertEquals(Kind.FRIENDS, stored.kinds[1])
            assertEquals(Kind.WORK, stored.kinds[2])
            assertTrue(stored.activities.single { it.id == 1L }.archived)
            assertEquals(1000L, stored.trial!!.activatedAt)
            assertEquals(Totals(3_000_000, 3_000_000), stored.trial!!.baseline)
            assertEquals("Europe/Rome", stored.trial!!.zoneId)
            ...
            assertEquals(0, java.math.BigDecimal("0.0525").compareTo(credits.spent))
        } finally {
            db.close()
            context.deleteDatabase(name)
        }
    ...
    @Test fun versionTwoUpgradeAddsCostsWithoutChangingTheTrialPrice() = runBlocking {
        ...
        SQLiteDatabase.openOrCreateDatabase(path, null).use { old ->
            ...
            val setup = schema.getJSONArray("setupQueries")
            for (i in 0 until setup.length()) old.execSQL(setup.getString(i))
            old.execSQL("INSERT INTO activities VALUES (1, ' Cooking ', 'NEUTRAL', 1, 0), (2, 'TRAVEL', 'WORK', 1, 0), (3, 'Friends', 'NEUTRAL', 1, 1), (4, 'Sleep', 'NEUTRAL', 1, 0), (5, 'Renamed project', 'SIDE', 1, 0), (6, ' Chores ', 'NEUTRAL', 1, 1)")
            old.execSQL("INSERT INTO records VALUES (1, 1, 1000, 3001000, '')")
        ...
        val db = Room.databaseBuilder(context, CurrencyDatabase::class.java, name)
            .addMigrations(CurrencyDatabase.MIGRATION_2_3, CurrencyDatabase.MIGRATION_3_4, CurrencyDatabase.MIGRATION_4_5).build()
        try {
            val stored = CurrencyRepository(db).load()
            ...
            assertEquals(Kind.CHORES, stored.kinds[6])
            assertTrue(stored.activities.single { it.id == 6L }.archived)
            assertTrue(stored.activities.single { it.id == 3L }.archived)
            assertEquals("same", stored.sync!!.hash)
            assertEquals("content://test/backup", stored.sync!!.uri)
            val credits = Economy.account(Economy.totals(stored.snapshot, stored.kinds, 1000, 3001000), stored.trial!!)
            assertEquals(0, java.math.BigDecimal("0.39375").compareTo(credits.spent))
        } finally {
            db.close()
            context.deleteDatabase(name)
        }
    ...
    @Test fun upgradePreservesTrialAndRecordsAndRefreshesArchiveFlags() = runBlocking {
        ...
        val db = Room.databaseBuilder(context, CurrencyDatabase::class.java, name)
            .addMigrations(CurrencyDatabase.MIGRATION_1_2, CurrencyDatabase.MIGRATION_2_3, CurrencyDatabase.MIGRATION_3_4, CurrencyDatabase.MIGRATION_4_5).build()
        try {
