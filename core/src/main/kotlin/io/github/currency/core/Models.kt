...
enum class Kind { WORK, SIDE, CHORES, LEISURE, COOKING, TRAVEL, FRIENDS, SLEEP, NEUTRAL }

fun classify(activity: Activity): Kind = when {
    activity.name.trim().equals("chores", ignoreCase = true) -> Kind.CHORES
    ...
    activity.name.trim().lowercase(java.util.Locale.ROOT) in setOf("friends", "people") -> Kind.FRIENDS
